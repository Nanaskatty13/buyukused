...
const ProductCard = memo(({ product }) => {
  ...
  const isPhone = useMemo(() => normalizedCategory === "Phones", [
    normalizedCategory,
  ]);
  ...
  const categorySpecs = useMemo(() => {
    ...
    if (normalizedCategory === "Laptops") {
      ...
      if (product.bedrooms)
        specs.push({ icon: "🛏️", label: `${product.bedrooms} bedrooms` });
      if (product.bathrooms)
        specs.push({ icon: "🚿", label: `${product.bathrooms} baths` });
      if (product.sqm) specs.push({ icon: "📐", label: `${product.sqm} sqm` });
      if (product.propertyType)
        specs.push({ icon: "🏠", label: product.propertyType });
      if (condition) specs.push({ icon: "📋", label: condition });
    } else {
      if (brand) specs.push({ icon: "🏷️", label: brand });
      if (model) specs.push({ icon: "📟", label: model });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="location-text">{cityOnly || "Ghana"}</span>
          <span>•</span>
        ...
        {sellerObj?.phone && (
          ...
          >
            ...
            >
              <i className="fas fa-comment-dots" style={{ fontSize: "10px" }} />
              CHAT
            </button>
            <button
              onClick={handleCall}
