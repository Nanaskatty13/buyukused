...
const SellerPage = () => {
    ...
    const fetchSellerData = async () => {
      ...
      try {
        ...
        if (!profileData?.success || !profileData?.seller) {
          throw new Error(profileData?.message || "Failed to load seller profile.");
        }

        const sellerData = profileData.seller;

        // ─── Debug: check if isVerified exists ────────────────
        console.log("✅ isVerified from API:", sellerData.isVerified);
        ...
        setSeller({
          ...
          isVerified: sellerData.isVerified === true,
        });
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Please sign in or create a BuyUKUsed account before viewing
  ...
  const memberDate = seller.memberSince ? new Date(seller.memberSince) : null;
  const memberSince =
  ...
  const memberDuration = seller.memberSince ? timeAgo(seller.memberSince) : "N/A";

  const lastSeenDate = seller.lastActive || seller.lastSeen || null;
  const lastSeen = lastSeenDate ? timeAgo(lastSeenDate) : null;

  const productCount = seller.productsCount || products.length || 0;

  const sellerImage = getSellerImageUrl(seller.avatar);
  ...
  return (
    <>
      ...
      >
        ...
        >
          {/* ==================================================
              AVATAR (with optional overlay badge)
          ================================================== */}
          ...
          >
            ...
            >
              {sellerImage && !imageError ? (
                <img
                  ...
                  onError={(event) => {
                    console.error("❌ Seller profile image failed:", sellerImage);
                    setImageError(true);
            ...
            {/* Verified badge overlay on avatar (optional – keep or remove) */}
            {isVerified && (
              <div
                style={{
                  position: "absolute",
          ...
          >
            {/* ─── Name + Verified Badge (clearly to the right) ─── */}
            <div
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              ...
              {/* Badge appears to the right of the name, with a label */}
              {isVerified && (
                <VerifiedBadge
                  size={28}
                  showLabel
            ...
            >
              ...
              {memberDuration && memberDuration !== "N/A" && <> ({memberDuration})</>}
            </div>
            ...
            <button
              ...
              style={{
                padding: "10px 24px",
                background: seller.phone ? "#25D366" : "var(--gray-300)",
                color: "white",
                border: "none",
                borderRadius: "var(--radius-full)",
                fontWeight: 700,
                fontSize: "15px",
                cursor: seller.phone ? "pointer" : "not-allowed",
                display: "inline-flex",
                alignItems: "center",
                gap: "8px",
              }}
            >
              ...
              {seller.phone ? "Contact Seller" : "No Phone Number"}
            </button>
          </div>
        </div>
